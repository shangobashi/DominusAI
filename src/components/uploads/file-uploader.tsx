...
export function FileUploader({
  ...
  const getFileIcon = (fileName: string) => {
    ...
    switch (extension) {
      ...
      case 'txt':
        return <div className="w-8 h-8 rounded bg-gray-500/20 text-gray-500 flex items-center justify-center">TXT</div>;
      default:
    ...
    <div className="space-y-4">
      ...
      <div
        ...
        className={`border-2 border-dashed border-border rounded-lg p-6 text-center cursor-pointer transition-colors ${
          isDragActive ? 'bg-primary/5 border-primary' : 'hover:bg-background'
        }`}
      >
        <input {...getInputProps()} />
